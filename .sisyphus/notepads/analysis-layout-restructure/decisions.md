...
- 2026-04-13: Task 5 の tabs 化では `DetailPanel` を PL内訳タブへそのまま移し、既存の `summaryRows/detailRows/weakLinkTarget` 連携は触らない方針にした。差異分解クリックの既存挙動を壊さず、MajorAccountSummary だけを安全に除去するため（KISS, DRY）。
